...
내가 지금까지 작성한 개발 블로그 + 포트폴리오 + 기타 내 소개를 한 곳에서 보여주고 싶어서 Jekyll 블로그를 만들어야겠다는 생각이 들었다.
그리고 Velog에 올린 글이 자동으로 내 개인 블로그(GitHub Pages)에 반영된다면 어떨까?
이번 글에서는 Velog → Jekyll → GitHub Pages 자동 배포 시스템을 만들어본 과정을 정리했다.
한 번 설정하면 이후엔 Velog에 글을 올리는 것만으로 블로그가 자동 갱신된다.

📌 목표 구조
Velog → (RSS 파싱)
      ↳ Python 변환 스크립트 (이미지 포함)
           ↳ GitHub Push
                ↳ GitHub Actions → Jekyll 빌드 & 배포
                
### ⚙️ 1. Jekyll 블로그 초기 세팅

먼저 GitHub Pages용 Jekyll 블로그를 생성했다.
...
로컬 서버(http://127.0.0.1:4000/)에서 테마와 구성을 확인한 뒤,
aneomagig.github.io라는 GitHub 저장소를 만들어 연결했다.
```
git init
...
### 🧩 2. Velog 글을 자동으로 변환하는 Python 스크립트 작성

Velog는 RSS 피드를 제공하므로,
이를 기반으로 포스트를 Markdown으로 변환하는 스크립트를 작성했다.
```
...
이 스크립트는
- RSS에서 포스트 내용을 파싱
- Velog 이미지(velcdn.com)를 로컬로 다운로드
- Jekyll용 _posts/YYYY-MM-DD-title.md 파일로 저장
까지 자동 수행한다.

### ⚙️ 3. GitHub Actions로 완전 자동화

매일 새벽에 이 스크립트가 실행되도록 GitHub Actions를 설정했다.
아래 파일을 생성한다.
...
### 🧩 이미지 깨짐 문제와 해결 과정
처음엔 Velog에서 가져온 글들이 Jekyll 블로그에 잘 표시되었지만, 본문에 포함된 이미지들이 전부 깨져 있었다. 브라우저 콘솔을 열어보니 경로가 이렇게 되어 있었다.
/assets/images/hosooinmymind/image.png  → 404 (Not Found)

이유를 분석해보니 Velog RSS에서 제공하는 <img> 경로가 절대주소(https://velog.velcdn.com/...)인데, RSS 변환 후 단순히 Markdown만 저장하면 Jekyll이 빌드 시 이 외부 이미지를 그대로 복사하지 못하는 게 원인이었다.

#### ❌ 잘못된 방식
처음엔 단순히 <img src>를 유지했지만, GitHub Pages는 velog.velcdn.com 도메인에서 이미지를 불러올 때 CORS 정책이나 HTTPS→HTTP 혼합 콘텐츠 차단으로 인해 이미지를 렌더링하지 못했다.

#### ✅ 해결 방법: 이미지 다운로드 & 경로 변환
결국 Python 스크립트에서 이미지 파일을 로컬로 다운로드한 뒤, 모든 <img>의 src를 블로그 내부 경로로 바꾸는 방식으로 해결했다.
...
이 코드는 다음을 수행한다:
① RSS의 <img> 태그에서 원본 src 추출	https://velog.velcdn.com/...
② 이미지 파일을 assets/images/USERNAME/ 아래에 저장	로컬 정적 파일로 변환
③ HTML 내 src 경로를 /assets/... 로 교체	Jekyll에서 로드 가능하게 변경

결과적으로 GitHub Pages에서도 모든 이미지가 깨지지 않고 표시되었다.
특히 requests.get()에 User-Agent 헤더를 추가한 것이 중요했다.
Velog의 CDN이 기본 요청(헤더 없는 요청)을 차단하기 때문이다.

#### 🧠 배운 점
단순한 RSS 크롤링만으로는 완전한 블로그 백업이 되지 않는다.
이미지 리소스를 함께 관리해야 “오프라인에서도 독립적인 블로그”가 가능하다.
Jekyll과 같은 정적 블로그는 모든 리소스가 /assets 경로에 포함되어야 함을 기억하자.

### 💡 4. 완성된 자동화 흐름
🕓 매일 1회 GitHub Actions 트리거
🪄 Python 스크립트 실행 -> Velog RSS 파싱 & 이미지 로컬 저장
💾 _posts/ 갱신 -> 새 글 자동 추가
🚀 GitHub Pages 빌드 -> Jekyll 사이트 재배포 완료

### 🎉 결과
이제 Velog에 새 글을 올리면 다음 날 자동으로 내 Jekyll 블로그에도 올라온다.
이미지도 깨지지 않고, 원문 구조 그대로 유지된다.
Velog와 GitHub Pages를 함께 운영하니 개발 기록과 개인 블로그를 하나의 생태계처럼 다룰 수 있게 되었다.

## Part 2. Discord 알림 설정
대 자동화 시대에 매일 내가 벨로그를 동기화하는 건 말이 안 된다고 생각
자동으로 하루에 한 번씩 동기화하고 디스코드 알림까지 오도록 설정했다.

### 1️⃣ 목표
- Velog RSS 피드를 이용해 자동으로 포스트를 생성
- 각 글의 이미지도 함께 다운로드하여 _posts 폴더에 저장
- GitHub Actions로 매일 자동 실행
- 마지막으로 디스코드 웹훅 알림을 보내도록 설정

### 2️⃣ Velog RSS 크롤링 스크립트
RSS 주소 예시 👉 https://v2.velog.io/rss/hosooinmymind
이 피드를 읽어와 Jekyll용 마크다운 포스트로 변환한다.
```
...
### 3️⃣ GitHub Actions 자동 실행 세팅
.github/workflows/velog-sync.yml
자동으로 위 스크립트를 실행하는 워크플로 파일이다.
```
...
### 5️⃣ Discord Webhook 설정

디스코드 서버에서 → 서버 설정 → 통합 → Webhook → 새 웹훅 만들기
복사한 URL을 GitHub Secrets에 추가
🔹 GitHub → Settings → Secrets → Actions → New repository secret

### 6️⃣ 테스트 실행
Actions → Velog Sync → Run workflow 클릭 
모든 단계가 초록색 ✅ 이면 성공.
디스코드에는 이런 알림이 도착한다:
![](/assets/images/hosooinmymind/images/hosooinmymind/post/c51c6155-3418-4ed1-a67a-708c544d589d/image.png)

## ✨ 마무리
이제 Velog에 글을 올리면
GitHub Pages에 자동 반영되고,
매일 자정엔 새 글이 자동으로 업데이트된다.
